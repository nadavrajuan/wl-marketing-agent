...
export async function GET(req: NextRequest) {
  ...
  const url = type
    ? `${AGENT_URL}/agent/api/research/templates?type=${encodeURIComponent(type)}`
    : `${AGENT_URL}/agent/api/research/templates`;
...
export async function POST(req: NextRequest) {
  ...
  const res = await fetch(`${AGENT_URL}/agent/api/research/templates`, {
    method: "POST",
